...
- Fonts: the image bundles **DejaVu** (Bitstream Vera/Arev license) and **Carlito** (SIL OFL 1.1). No proprietary font is shipped. A template that references **Calibri** (Microsoft) is rendered with **Carlito**, its metric-compatible clone, which LibreOffice substitutes automatically, so line and page breaks stay close to Word's instead of falling back to the wider DejaVu. A template in **Cambria** has no such clone installed: add **Caladea** (`fonts-crosextra-caladea`) the same way if you need it.
